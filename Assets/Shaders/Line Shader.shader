    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            v2f vert (appdata v)
            {
                ...
                float4 xyz = float4(
                        v.vertex.x,
                        // mul().x-1 is the object's x position. Idk why /2 - 1 is necessary
                        v.vertex.y + 100*_Magnitudes[floor((o.vertexInWorldCoords.x-(mul(unity_ObjectToWorld, float4(0,0,0,1)).x - 1.0))*(_NumPartitions/2 - 1))],
                        v.vertex.z,
                        1.0
                );
