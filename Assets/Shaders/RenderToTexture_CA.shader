Shader "Custom/RenderToTexture_CA"
{
	Properties
	{
		...
		_PosSize("PosSize", Float) = 0
		_test("test",Float) = 0
		_PS("PS", Float) = 0.01 // PixelSize A.K.A. how big the pixel will be.
		_Changed("Changed", Int) = 1 // Sub for bools
		...
		SubShader
	{
		...
		Pass
		{
			...
			uniform float _PosSize = 0;
			uniform float _PosY[76];
			uniform float _PosX[76];
			uniform float _PS;
			uniform float _test;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				// new position check
				if (_Changed == 0) {
					for (int p = 0; p <= _PosSize; p++) {
						if (p == _PosSize) {
							_Changed = 1;
						}
						if (cx > _PosX[p] - _PS && cx < _PosX[p] + _PS) {
							if (cy > _PosY[p] - _PS && cy < _PosY[p] + _PS) {
								_test = texel.x;
